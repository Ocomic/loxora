...
The header uses the exact user-supplied Loxora raster lockup. The supplied 1254×1254 original is retained, and a pixel-exact derived crop removes only its empty square canvas for stable header rendering. The cyan, blue, violet, and near-black interface palette follows that supplied identity. Result Receipts and Core Context results use ordinary accessible block sections instead of browser-native output widgets; the sticky shell uses opaque paint surfaces so successful output cannot fragment or corrupt surrounding presentation. A completed Context build is distinguished from an unavailable future action, and reset invalidates any browser-local result that is no longer backed by server Context readiness. Follow-up fixture preparation is isolated from the accepted canonical transition: if preparation fails after acceptance, canon remains accepted and the Guided UI offers safe Resume recovery instead of misreporting the review as failed.
